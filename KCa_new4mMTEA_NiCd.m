...
opts = optimset('Display','off');%,'Algorithm','levenberg-marquardt');
io = 4440;
fW = 4600:9380;%9180;%6304;%4550:9380 fo single % 4450:9380 for double
...
%                 p0 = [ 0 ,              0.01,         0,              0.5];
%                 lb = [0   ,              0.005   ,     0,            0 ];
%                 ub = [prctile(yy,99)*1.5, 0.5 ,         1,            Inf];
%                 KAp{n}(e,t,:) = lsqcurvefit(fun,p0,X(1:length(fW)),yy',lb,ub,opts);


%                 KAp{n}(e,t,:) = lsqcurvefit(fun,p0,X(1:length(fW)),data{n}(fW,1,t,3,e)',...
%                     [prctile(yy,99) ,0.005,0,0],[prctile(data{n}(fW,1,t,3,e),99)*1.5,inf,1,inf],opts);
            else
                KAp{n}(e,t,:) = nan;
            end
        end
...
%         tp0 = [0.1, 0, -20, 8, 20];%max(Yd) 
%         atp{n}(e,:) = lsqcurvefit(tfun,tp0,xt,Yd,[0,0,-inf,-inf,0],[inf,inf,inf,inf,inf],opts);
%         tfun = @(p,x) (p(1) - p(2))./(1+exp((p(3) - x)/p(4))) + p(2);
        tp0 = [0.1, 0, -20, 1];%max(Yd)
%         atp{n}(e,:) = lsqcurvefit(tfun,tp0,xt,Yd,[0,0,-inf,-inf],[inf,inf,inf,0],opts);
    end
end
...
%         tfun = @(p,x) (p(1) - p(2))./(1+exp((x - p(3))/-p(4)))./(1+exp((x - (p(3) + p(5)))/p(4))) + p(2);
%         tp0 = [0.1, 0, -20, 8, 20];%max(Yd)
%         itp{n}(e,:) = lsqcurvefit(tfun,tp0,xt,Yd,[0,0,-inf,-inf,0],[inf,inf,inf,inf,inf],opts);
%     end
% end
% 
% axs22 = axes('Position',[0.05 0.05 0.4 0.4]);
% itplt = gobjects(1,3);
% for n=1:length(neuron)
%     Ym = mean(KAp{n}(:,:,4));
%     err = nanstd(KAp{n}(:,:,4))/sqrt(size(KAp{n}(:,:,4),1));
%     Ye = [Ym + err;Ym - err];
...
%     itplt(n) = plot(-80:30,tfun(median(itp{n}),-80:30),'Color',colorss(n,:));hold on
% end
% % legend(names)
% axs22.Title.String = 'Inactivation time constant';
% % axs22.YLim(1) = 0;
...
% 
% 
% axs23 = axes('Position',[0.55 0.05 0.4 0.4]);
% for n=3%1:length(neuron)
%     for e=1:size(KAp{n},1)
%         Y = KAp{n}(e,:,4);
%         scatter(command{n}(1,:),Y,20,'d','MarkerFaceColor',colors(e,:),'MarkerEdgeColor',colors(e,:));hold on
%     end
%     plot(command{n}(1,:),mean(KAp{n}(:,:,4)))
% end
