...
%% plotting full inactivaiton protocols

colors = makecolor;
for n = 1:3
    close(findobj(0,'Name', [neuron{n} ' inactivation']))% 
    fig = figure('Name',[neuron{n} ' inactivation'],'NumberTitle','off');
    for p = 1:size(dataif{n},5)
        subplot(2,3,p)
        for v = 1:15
            plot(dataif{n}(1:10:end,3,v,3,p),'Color',colors(v,:));hold on
        end
    end
    legend(string(commandi{1}(1,:)))
end
...
% %%
% 
% close(findobj(0,'Name', 'DK vs Kca vs KA'))% 
% figure('Name','DK vs Kca vs KA','NumberTitle','off','Position',[2000 200 1800 800]);
% 
% funcp = @(p,x) p(1).*(1 - e.^((x - p(1))./-p(2)));
% 
% kca = squeeze(KCa(:,end,:));
% kca = kca./repmat(kca(690,:),size(kca,1),1);
% pkca = plot((1:length(kca))-207,nanmean(kca,2));hold on
% 
% dk = squeeze(DK(:,end,:));
% dk = dk./repmat(dk(690,:),size(dk,1),1);
% p = [0.6 20000];
% % pdk = plot((1:length(dk))-207,nanmean(dk,2),'r:');hold on
% % dk(357:end,:) = dk(357:end,:) + repmat(funcp(p,0:(length(dk)-357))',1,size(dk,2));
% pdk = plot((1:length(dk))-207,nanmean(dk,2));hold on
% 
% 
% ka = squeeze(data{1}(fW2,1,end,3,:));
% ka = ka./repmat(ka(260,:),size(ka,1),1);
% plot(nanmean(ka,2));hold on
% 
% legend('kca','dk','ka')
% 
% %%
...
%     delete(pf)
% end
% funcp = @(p,x) 1 - e.^((x - p(1))./-p(2));
% 
% pf = plot(144:5000,funcp([144, 5500],144:5000));

save('KA','data','command')